...
class Router
{
    ...
    public function handleRoute(): void
	{
		...
		$callback = Route::getCallback($method, $path);
        try {
            $middlewares = array_merge(Route::getMiddleware($method, $path) ?? [], Middleware::getGlobalMiddlewares());
        } catch (Exception $e) {
            Throw new Exception($e->getMessage() . " in the route '$path'.");
        }

        // Middleware and callback exists.
		if ($middlewares !== null && $callback !== null) {
	...
	{
		$callback = Route::getCallback('get', '/error');
